    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-6">
          {federations.map((partner) => (
            <div key={partner.id} className={`bg-white rounded-xl border p-6 ${partner.isPartner ? "border-orange/30 ring-1 ring-orange/10" : "border-gray-100"}`} data-testid={`card-partner-${partner.id}`}>
              <div className="flex items-start gap-4">
                <div className={`w-16 h-16 rounded-xl flex items-center justify-center flex-shrink-0 ${partner.isPartner ? "bg-orange/10" : "bg-gray-100"}`}>
                  <Award className={`w-8 h-8 ${partner.isPartner ? "text-orange" : "text-gray-400"}`} />
                </div>
                <div className="flex-1">
                  ...
                  {partner.websiteUrl && (
                    <a href={partner.websiteUrl} target="_blank" className="inline-flex items-center gap-1 text-sm font-semibold text-orange hover:text-orange-600 transition-colors" data-testid={`link-partner-website-${partner.id}`}>
